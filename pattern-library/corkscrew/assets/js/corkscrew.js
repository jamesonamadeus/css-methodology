var Pinot = {
  ...
  addExamplesDropdown: function() {

    if( Corkscrew.examples && Corkscrew.examples.length ) {
      ...
      var nav = document.querySelectorAll("#cs-site-nav")[0],
          ...
          dropDownEl.innerHTML ='<a href="#cs-examples" id="cs-drop-trigger">examples</a>';
          dropDownMenu.setAttribute('id', 'cs-examples');
          dropDownMenu.classList.add('cs-site-nav-menu');

          for ( var i = 0; i < Corkscrew.examples.length; i = i + 1 ) {

            templateItem = document.createElement("li");
            templateItem.innerHTML = '<a href="' + Corkscrew.examples[i].location + '">' + Corkscrew.examples[i].name + '</a>';
  ...
  buildGuideSections: function() {
    ...
    if( Corkscrew.sections.length ) {
      ...
      for ( var i = 0; i < Corkscrew.sections.length; i = i + 1 ) {
        ...
        if ( section.startsWith(isPrefixed) ) {
          navItem.innerHTML = '<b class="cs-nav-title">' + section.replace(/TITLE:/g, '') + '</b>';
        }
        else {
